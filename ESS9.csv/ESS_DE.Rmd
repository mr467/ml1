...
```{r, data preparation, include=TRUE}
# defining subset for study and models. These factors will be used in the Poisson and Neural Network models below. Keeping the helping columns to assure to be fine
ess9_subset <- c("nwspol", "polintr", "trstprl", "eisced", "eduyrs","stfeco", "stfgov", "stfdem", "gndr", "agea","rlgdgr","netusoft", "psppsgva","infqbst" ,"grspnum", "yrpy")
```
...
factors <- c("polintr","trstprl", "eisced", "stfeco", "stfgov", "stfdem", "gndr", "rlgdgr", "psppsgva", "infqbst", "netusoft")
...
```{r}
set.seed(123)
t(ess9_ann2)%*%as.matrix(ess9_ann2)
ess9_ann_net <- neuralnet(netusoft ~ ., train, hidden = c(4,3))
```
...
```{r}
set.seed(142)
formula = netusoftsetosa + netusoftversicolor + netusoftvirginica ~ Sepal.Length + Sepal.Width + Petal.Length + Petal.Width
...
plot(best_model)
```
A word of caution: the `ess9_ann` dataset is almost to easy to separate, so it is very easy to get stuck in local minimums i.e. your results may depend a lot on the random seeds chosen.
