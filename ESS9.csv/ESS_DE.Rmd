...
ess9_binary <- ess9_de[, c("postonline", "agea.na", "polint", "gov.allows", "trustparl", "polscale.na")]
...
glm_pol.0 <- glm(postonline ~ agea.na, data = ess9_binary.na,
                 family = "binomial")
summary(glm_pol.0)
```
...
glm_pol.1 <- glm(postonline ~ agea.na + polint, data = ess9_binary.na,
                 family = "binomial")
summary(glm_pol.1)
```

```{r}
glm_pol.2 <- glm(postonline ~ agea.na + polint + polscale.na, data = ess9_binary.na,
                 family = "binomial")
summary(glm_pol.2)
```

```{r}
glm_pol.1 <- glm(postonline ~ agea.na + polint + polscale.na + trustparl, data = ess9_binary.na,
                 family = "binomial")
summary(glm_pol.1)
```
...
```{r}
d.ess9_supportvec <- ess9_de[, c("postonline", "agea.na", "polint", "gov.allows", 
                                 "trustparl", "polscale.na", "scale_cat", "nwspol_na")]
...
```{r}
d.ess9_supportvec %>%
  ggplot(aes(x = agea.na, y = nwspol_na, color = scale_cat)) +
  geom_point() +
  ylim(0, 250)
```
...
```{r}
d.ess9_supportvec %>%
  ggplot(aes(x = agea.na, y = trustparl, color = scale_cat)) +
  geom_point()
```
...
```{r}
plot(ess_de_svm, train, agea.na ~ nwspol_na)
```
