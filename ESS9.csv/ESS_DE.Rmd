...
```{r Your [pay/pensions/social benefits], which frequency do you know best, include = FALSE}
...
```{r recode Variable resp. clean, results = FALSE, include = FALSE}
...
```{r all variables that i want to include, include=FALSE}
#DV
...
```{r create a DV where no outlier, include=FALSE}
ess9_de$nwspol.outlier <- ess9_de$nwspol.na
ess9_de["nwspol.outlier"][ess9_de["nwspol.outlier"] >= 400] <- NA
```

```{r create a subset of data with only the variables mentioned above, include=FALSE}
...
```{r boxplots for the categorial variables w/ outliers and without outliers, include=FALSE}
par(mfrow=c(2,3))
...
[**Interpretation**]{.underline}: By increasing the age by one year, the risk in odds will be increasing by approx. 1 time to post something political online.\
...
```{r}
ess_de_svm.1 <- svm(scale_cat ~ ., train, kernel = "linear", scale = TRUE, cost = 0.1)
...
```{r set training data, include=FALSE}
set.seed(123)
