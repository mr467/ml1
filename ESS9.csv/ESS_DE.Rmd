...
```{r}
ggplot(ess9_de, aes(eduade3.na, nwspol.na)) + 
  geom_point() +
  ylim(0, 500)
```
...
```{r}
plot(nwspol.na ~ eduade3.na, data = ess9_de, main = "Minutes spent on Political News")
abline(lm_news.1)
```
...
plot(nwspol.na ~ eduade3.na, data = ess9_de, 
...
```{r}
lm_news.2 <- lm(nwspol.na ~ eduade3.na + age.na, data = ess9_de)
summary(lm_news.2)
```
...
```{r}
plot(nwspol.na ~ eduade3.na + age.na, data = ess9_de, main = "Minutes spent on Political News")
abline(lm_news.2)
```

```{r}
lm_news.3 <- lm(nwspol.na ~ eduade3.na + age.na + gndr.fac, data = ess9_de)
summary(lm_news.3)
```
